...
CREATE TABLE Leiter (
                        id BIGINT PRIMARY KEY GENERATED ALWAYS AS IDENTITY,                        name VARCHAR(255) NOT NULL
);
...
CREATE TABLE Tier (
                      ...
                      FOREIGN KEY (naturschutzgebiet_id) REFERENCES Naturschutzgebiet(id)  -- Verweist nun korrekt auf den Primärschlüssel
);
...
CREATE TABLE Trackinggeraet (
                                id BIGINT PRIMARY KEY GENERATED ALWAYS AS IDENTITY,                                seriennummer VARCHAR(50) UNIQUE NOT NULL,
                                tier_id BIGINT,
                                FOREIGN KEY (tier_id) REFERENCES Tier(id)
);

CREATE TABLE Wanderung (
                           id BIGINT PRIMARY KEY GENERATED ALWAYS AS IDENTITY,                           datum DATE NOT NULL,
...
CREATE TABLE gplnt_tbl (
                           gid BIGINT PRIMARY KEY GENERATED ALWAYS AS IDENTITY,
...
CREATE TABLE expnd_spread (
                              eid BIGINT PRIMARY KEY GENERATED ALWAYS AS IDENTITY,
                              gref BIGINT,
